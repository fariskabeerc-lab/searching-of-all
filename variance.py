...
st.title("🏪 Sales QTY Check - Outlet Comparison")
...
def load_all_data(files_list):
    ...
    # Convert monthly columns to numeric and calculate 'Total Sales'
    ...
    # Calculate Avg Monthly Sale (using 10 months as placeholder, as in your original logic)
    master_df['Avg Monthly Sale'] = (master_df['Total Sales (Qty)'] / 10 + 1).astype(int)
...
if password == "123123":
    ...
    if search_term:
        ...
        if not filtered_df_item.empty:
            ...
            st.subheader(f"📦 Results for: **{selected_item_name}** (Total Sales Period Qty)")

            # --- Group/Aggregate Data for the Bar Chart ---
            # Sum up sales by Outlet (important if the same item code appears multiple times per outlet)
            outlet_sales = final_item_df.groupby('Outlet').agg(
                {'Total Sales (Qty)': 'sum', 'Avg Monthly Sale': 'mean', 'Item Code': 'first', 'Items': 'first'}
            ).reset_index()
            ...
            if not outlet_sales.empty:
                ...
                st.info(f"**Overall Avg Monthly Sale per Outlet:** {int(avg_sale)} units (Calculation based on total sales / 10 + 1)")

                # --- Horizontal Bar Chart (Outlet Comparison) ---
                st.markdown("### 📊 Outlet Comparison")
                fig = px.bar(
